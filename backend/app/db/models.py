...
class RepositorySearchDocument(Base):
    ...
    github_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("catalog_repositories.github_id", ondelete="CASCADE"),
        primary_key=True,
    )
...
class RepositorySignalSnapshot(Base):
    ...
    __table_args__ = (Index("ix_signal_snapshot_repo_captured", "github_id", "captured_at"),)
...
class RepositoryProvenance(Base):
    ...
    __table_args__ = (Index("ix_provenance_repo_field", "github_id", "field_name"),)
...
class RepositoryExternalLink(Base):
    ...
    verification: Mapped[str] = mapped_column(String(30))
    provenance_url: Mapped[str] = mapped_column(String(2048))
    observed_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    __table_args__ = (
        UniqueConstraint("github_id", "source", "external_id", name="uq_external_link"),
    )


class ExternalEvidenceItem(Base):
    __tablename__ = "external_evidence_items"
    id: Mapped[int] = mapped_column(primary_key=True)
    github_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("catalog_repositories.github_id", ondelete="CASCADE")
    )
    source: Mapped[str] = mapped_column(String(30))
    external_id: Mapped[str] = mapped_column(String(255))
    kind: Mapped[str] = mapped_column(String(50))
    title: Mapped[str] = mapped_column(String(300))
    excerpt: Mapped[str | None] = mapped_column(String(1200))
    ...
    __table_args__ = (
        UniqueConstraint("github_id", "fingerprint", name="uq_evidence_fingerprint"),
        Index("ix_evidence_repo_observed", "github_id", "observed_at"),
        Index("ix_evidence_retention", "observed_at"),
    )


class RepositorySourceState(Base):
    """Shared normalized cache and collection health; no raw HTTP responses."""

    __tablename__ = "repository_source_states"
    github_id: Mapped[int] = mapped_column(
        BigInteger,
        ForeignKey("catalog_repositories.github_id", ondelete="CASCADE"),
        primary_key=True,
...
class RepositorySourceSnapshot(Base):
    __tablename__ = "repository_source_snapshots"
    id: Mapped[int] = mapped_column(primary_key=True)
    github_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("catalog_repositories.github_id", ondelete="CASCADE")
    )
    ...
    __table_args__ = (
        ...
        Index("ix_source_snapshot_repo_time", "github_id", "captured_at"),
    )


class RepositoryChangeEvent(Base):
    __tablename__ = "repository_change_events"
    id: Mapped[int] = mapped_column(primary_key=True)
    github_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("catalog_repositories.github_id", ondelete="CASCADE")
    )
    ...
    evidence_id: Mapped[int | None] = mapped_column(
        ForeignKey("external_evidence_items.id", ondelete="SET NULL")
    )
    ...
    fingerprint: Mapped[str] = mapped_column(String(64))
    __table_args__ = (
        UniqueConstraint("github_id", "fingerprint", name="uq_change_fingerprint"),
        Index("ix_change_repo_time", "github_id", "occurred_at"),
    )
