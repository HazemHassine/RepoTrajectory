...
async def main() -> None:
    ...
    async with SessionLocal() as session:
        ...
        session.add(
            RepositoryEmbedding(
                github_id=repo.github_id,
                embedding_version=cfg.ai_embedding_version,
        ...
        result = await hybrid_search(
            session, "python agent", settings=cfg, ai_provider=MockSemanticProvider()
        )
        assert result["semantic_available"] and result["items"][0]["github_id"] == repo.github_id
        result = await hybrid_search(
            session, "python agent", settings=cfg, ai_provider=FallbackAIProvider()
        )
        ...
        observation = Observation(
            items=[
                Evidence(
                    ...
                    kind="release",
                    title="v1",
                    url="https://github.com/test/radar",
                    published_at=now,
                )
            ]
        )
    ...
    async with SessionLocal() as lock_session:
        await lock_session.scalar(
            select(IngestionJob).where(IngestionJob.id == one).with_for_update()
        )
    ...
    await finish(
        first or second or "", cfg, {"usage": {"prompt_tokens": 10, "completion_tokens": 5}}
    )
    assert (
        await reserve(
            cfg.model_copy(update={"ai_daily_request_limit": 1}),
            "test",
            "test",
            "scout",
            "c" * 64,
    ...
    assert (
        await reserve(
            cfg.model_copy(update={"ai_monthly_token_limit": 100}),
            "test",
            "test",
            "scout",
            "d" * 64,
    ...
    print(
        "PASS: pgvector, lexical fallback, evidence replay, indexes, "
        "SKIP LOCKED, AI budget concurrency"
    )
