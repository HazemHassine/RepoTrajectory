...
from app.db.models import CatalogRepository, QueryEmbeddingCache, RepositorySearchDocument
...
async def test_reciprocal_rank_fusion(db_session):
    ...
    results = await hybrid_search(
        db_session, query="vector database", ai_provider=ai_provider, limit=10
    )
...
async def test_lexical_fallback_when_ai_offline(db_session):
    ...
    results = await hybrid_search(db_session, query="resilient search", ai_provider=None, limit=5)
...
async def test_query_embedding_caching(db_session):
    ...
    emb1 = await get_or_create_query_embedding(
        db_session, query=query, ai_provider=ai_provider, settings=settings
    )
    ...
    emb2 = await get_or_create_query_embedding(
        db_session, query=query, ai_provider=ai_provider, settings=settings
    )
    assert emb1 == emb2
