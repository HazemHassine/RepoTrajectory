...
export default async function Rankings({
  ...
  try {
    const [repositories, metrics] = await Promise.all([
      api.repos(),
      api.rankings("momentum", window),
    ]);
  ...
  return (
    <main>
      <PageHeader
        title="Rankings"
        description="Rank repositories by momentum, health, delivery velocity, and community resilience."
        action={<WindowControl active={window} />}
      />
      <div className="mx-auto max-w-[1440px] px-5 py-6 md:px-8 xl:px-10">
        {unavailable ? (
          <EmptyState title="API service unavailable" />
        ) : records.length ? (
          <RankingWorkspace records={records} />
        ) : (
          <EmptyState />
        )}
      </div>
