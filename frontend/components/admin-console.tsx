...
export function AdminConsole() {
  ...
  return <main>
    <PageHeader title="Administration" description="Control plane for collection, ingestion, queue recovery, storage, and policy operations." action={<div className="flex items-center gap-2"><button onClick={() => void loadAdminData()} className="button-secondary"><ArrowPathIcon className="size-4" />Refresh</button><button disabled={busy === "logout"} onClick={() => void logout()} className="button-primary"><ArrowRightStartOnRectangleIcon className="size-4" />Sign out</button></div>} />
    <div className="mx-auto max-w-[1440px] space-y-6 px-5 py-6 md:px-8 xl:px-10">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[#222222] bg-[#0c0c0c] px-4 py-3"><div className="flex items-center gap-3"><div className="grid size-8 place-items-center rounded-full bg-[#161616] text-[#ccf200]"><ShieldCheckIcon className="size-4" /></div><div><p className="text-xs font-semibold">Signed in as {session.username}</p><p className="mt-0.5 text-[11px] text-[#9a9a9a]">HttpOnly session · expires {relativeDate(session.expires_at)}</p></div></div><span className="font-mono text-[10px] uppercase tracking-[.1em] text-[#9a9a9a]">Privileged actions are audited</span></div>

      {(notice || error) && <div className={`flex items-start justify-between gap-4 rounded-md border px-4 py-3 text-sm ${error ? "border-[#ffffff] bg-[#141414] text-[#ffffff]" : "border-[#ccf200] bg-[#161616] text-[#ccf200]"}`}><span>{error ?? notice}</span><button onClick={() => { setNotice(null); setError(null); }} className="text-xs font-semibold">Dismiss</button></div>}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(340px,.75fr)]">
        <section className="panel overflow-hidden"><SectionHeader title="Approved operations" description="Fixed server-side commands only. Arbitrary shell execution is intentionally unavailable." /><div className="grid gap-px bg-[#222222] sm:grid-cols-2">
        ...
        <section className="panel overflow-hidden"><SectionHeader title="Queue a repository" description="Pin an explicit GitHub repository for bounded full ingestion." /><form onSubmit={queueRepository} className="p-5"><label htmlFor="admin-repository" className="data-label">Owner/repository</label><input id="admin-repository" required autoComplete="off" pattern="[^/\s]+/[^/\s]+" value={repository} onChange={(event) => setRepository(event.target.value)} placeholder="fastapi/fastapi" className="mt-2 h-10 w-full rounded-md border border-[#262626] bg-[#090909] px-3 font-mono text-sm focus:border-[#ccf200] focus:outline-none" /><button disabled={busy === "queue"} className="button-primary mt-3 w-full"><PlusIcon className="size-4" />{busy === "queue" ? "Queuing…" : "Queue ingestion"}</button><p className="mt-3 text-[11px] leading-4 text-[#646464]">The request enters the durable worker queue. It does not execute inside the web request or grant write access to GitHub.</p></form></section>
      </div>

      <section className="panel overflow-hidden"><SectionHeader title="Work ledger" description={`${jobs.length} recent jobs with controlled retry and cancellation.`} action={<div className="w-36"><Combobox size="sm" value={jobFilter} onChange={setJobFilter} options={jobFilterOptions} placeholder="State" searchPlaceholder="Filter state…" clearable={false} /></div>} /><div className="overflow-x-auto"><table className="w-full min-w-[980px]"><thead><tr className="border-b border-[#222222] bg-[#0c0c0c]"><th className="table-head px-5 py-3">Job</th><th className="table-head px-4 py-3">State</th><th className="table-head px-4 py-3">Target</th><th className="table-head px-4 py-3">Attempts</th><th className="table-head px-4 py-3">Timing / error</th><th className="table-head px-4 py-3">Control</th></tr></thead><tbody>{visibleJobs.map((job) => <tr key={job.id} className="border-b border-[#222222] last:border-0"><td className="px-5 py-3"><p className="font-mono text-xs font-semibold">#{job.id}</p><p className="mt-1 text-[10px] text-[#9a9a9a]">{label(job.job_type)} · priority {job.priority}</p></td><td className="px-4"><StatusBadge status={label(job.status)} tone={jobTone(job.status)} /></td><td className="max-w-[250px] px-4 font-mono text-[11px] text-[#9a9a9a]"><span className="line-clamp-2">{String(job.payload.full_name ?? job.candidate_id ?? job.repository_id ?? "System")}</span></td><td className="px-4 font-mono text-xs">{job.attempts} / {job.max_attempts}</td><td className="max-w-[330px] px-4 text-xs text-[#9a9a9a]"><span className="line-clamp-2">{job.last_error ?? jobTiming(job)}</span></td><td className="px-4"><div className="flex gap-3">{["failed", "cancelled"].includes(job.status) && <button disabled={busy === `retry-${job.id}`} onClick={() => void jobAction(job, "retry")} className="text-xs font-semibold text-[#ccf200]">Retry</button>}{["queued", "failed"].includes(job.status) && <button disabled={busy === `cancel-${job.id}`} onClick={() => void jobAction(job, "cancel")} className="text-xs font-semibold text-[#ffffff]">Cancel</button>}</div></td></tr>)}</tbody></table>{visibleJobs.length === 0 && <div className="p-10 text-center text-sm text-[#9a9a9a]">No jobs match this state.</div>}</div></section>

      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(350px,.8fr)]">
        <section className="panel overflow-hidden"><SectionHeader title="Storage inventory" description="Normalized row counts as of the latest administrative refresh." /><div className="grid gap-px bg-[#222222] sm:grid-cols-2 lg:grid-cols-3">{Object.entries(summary.row_counts).map(([name, value]) => <div key={name} className="bg-[#0c0c0c] px-5 py-4"><p className="data-label">{label(name)}</p><p className="mt-2 font-mono text-xl font-semibold">{number.format(value)}</p></div>)}</div></section>
        <section className="panel overflow-hidden"><SectionHeader title="Runtime policy" description="Effective non-secret configuration. Credentials are never returned by this API." /><dl className="divide-y divide-[#222222] px-5"><Policy label="Collector" value={summary.configuration.collector_enabled ? "Enabled" : "Disabled"} /><Policy label="GitHub credential" value={summary.configuration.github_token_configured ? "Configured" : "Missing"} /><Policy label="Refresh cadence" value={`${summary.configuration.active_refresh_hours} hours`} /><Policy label="Discovery floor" value={`${number.format(summary.configuration.discovery_min_stars)} stars`} /><Policy label="Archive lookback" value={`${summary.configuration.gh_archive_hours_back} hours`} /><Policy label="Retention" value={`${summary.configuration.gh_archive_retention_days} days`} /><Policy label="Session lifetime" value={`${summary.configuration.admin_session_hours} hours`} /></dl></section>
      </div>

      <section className="panel overflow-hidden"><SectionHeader title="Administrative audit" description="Append-only record of sign-ins and accepted privileged operations." /><div className="overflow-x-auto"><table className="w-full min-w-[800px]"><thead><tr className="border-b border-[#222222] bg-[#0c0c0c]"><th className="table-head px-5 py-3">Time</th><th className="table-head px-4 py-3">Actor</th><th className="table-head px-4 py-3">Action</th><th className="table-head px-4 py-3">Target</th><th className="table-head px-4 py-3">Outcome</th></tr></thead><tbody>{audit.map((entry) => <tr key={entry.id} className="border-b border-[#222222] last:border-0"><td className="px-5 py-3 text-xs text-[#9a9a9a]">{new Date(entry.occurred_at).toLocaleString()}</td><td className="px-4 font-mono text-xs">{entry.actor}</td><td className="px-4 text-xs font-semibold">{label(entry.action)}</td><td className="px-4 font-mono text-[11px] text-[#9a9a9a]">{entry.target ?? "—"}</td><td className="px-4"><StatusBadge status={label(entry.outcome)} tone={entry.outcome === "rejected" || entry.outcome === "failed" ? "critical" : "positive"} /></td></tr>)}</tbody></table></div></section>
    </div>
  </main>;
}
...
function AdminLogin({ username, password, busy, error, onUsername, onPassword, onSubmit }: { username: string; password: string; busy: boolean; error: string | null; onUsername: (value: string) => void; onPassword: (value: string) => void; onSubmit: (event: FormEvent) => void }) {
  return <main><PageHeader title="Administration" description="Collection controls require a local administrative session." /><div className="mx-auto grid max-w-[1100px] gap-6 px-5 py-12 md:px-8 lg:grid-cols-[minmax(0,.9fr)_minmax(360px,.65fr)] xl:px-10"><section className="hairline-grid border border-[#222222] bg-[#050505] p-7 text-[#ffffff]"><div className="grid size-11 place-items-center border border-[#222222] bg-[#ffffff]/10 text-[#ccf200]"><ShieldCheckIcon className="size-5" /></div><p className="mt-6 text-[11px] font-semibold uppercase tracking-[.12em] text-slate-400">Operator boundary</p><h2 className="mt-2 text-2xl font-semibold tracking-[-.03em]">Privileged actions stay separate from research views.</h2><div className="mt-6 space-y-4"><SecurityPoint icon={<LockClosedIcon className="size-4" />} title="HttpOnly session" detail="The browser cannot read the signed session cookie." /><SecurityPoint icon={<KeyIcon className="size-4" />} title="CSRF and origin enforcement" detail="Every state change requires the session-bound token and an approved local origin." /><SecurityPoint icon={<CommandLineIcon className="size-4" />} title="Allowlisted commands" detail="The API exposes defined operations, never an arbitrary shell." /></div></section><form onSubmit={onSubmit} className="panel self-start overflow-hidden"><div className="border-b border-[#222222] p-5"><div className="grid size-9 place-items-center rounded-md bg-[#161616] text-[#ccf200]"><LockClosedIcon className="size-4" /></div><h2 className="mt-4 text-lg font-semibold">Operator sign-in</h2><p className="mt-1 text-xs leading-5 text-[#9a9a9a]">Credentials are checked by the private API and never stored in browser persistence.</p></div><div className="space-y-4 p-5">{error && <div className="flex gap-2 border border-[#ffffff] bg-[#141414] p-3 text-xs text-[#ffffff]"><ExclamationTriangleIcon className="mt-0.5 size-4 shrink-0" />{error}</div>}<label className="block"><span className="data-label">Username</span><input autoComplete="username" value={username} onChange={(event) => onUsername(event.target.value)} className="mt-2 h-10 w-full rounded-md border border-[#262626] bg-[#090909] px-3 text-sm focus:border-[#ccf200] focus:outline-none" /></label><label className="block"><span className="data-label">Password</span><input required type="password" autoComplete="current-password" value={password} onChange={(event) => onPassword(event.target.value)} className="mt-2 h-10 w-full rounded-md border border-[#262626] bg-[#090909] px-3 text-sm focus:border-[#ccf200] focus:outline-none" /></label><button disabled={busy} className="button-primary w-full"><ShieldCheckIcon className="size-4" />{busy ? "Verifying…" : "Enter administration"}</button><p className="text-center text-[10px] leading-4 text-[#646464]">Five rejected attempts trigger a temporary local lockout.</p></div></form></div></main>;
}

function AdminLoading({ error }: { error?: string | null }) {
  return <main><PageHeader title="Administration" description="Verifying the local operator session…" /><div className="mx-auto max-w-[900px] px-5 py-12 md:px-8"><div className="panel flex items-center gap-3 p-6 text-sm text-[#9a9a9a]">{error ? <ExclamationTriangleIcon className="size-5 text-[#ffffff]" /> : <ArrowPathIcon className="size-5 animate-spin text-[#ccf200]" />}{error ?? "Checking signed session and administrative service."}</div></div></main>;
}

function SecurityPoint({ icon, title, detail }: { icon: React.ReactNode; title: string; detail: string }) { return <div className="flex gap-3"><div className="mt-0.5 grid size-7 shrink-0 place-items-center border border-[#222222] bg-[#ffffff]/10 text-[#ccf200]">{icon}</div><div><p className="text-sm font-semibold">{title}</p><p className="mt-1 text-xs leading-5 text-slate-400">{detail}</p></div></div>; }
function Kpi({ label: title, value, detail, alert = false }: { label: string; value: string; detail: string; alert?: boolean }) { return <div className="bg-[#0c0c0c] px-5 py-4"><p className="data-label">{title}</p><p className={`mt-2 font-mono text-2xl font-semibold tracking-[-0.04em] ${alert ? "text-[#ffffff]" : ""}`}>{value}</p><p className="mt-1 text-[11px] text-[#646464]">{detail}</p></div>; }
function CommandCard({ icon, title, detail, command, busy, onRun }: { icon: React.ReactNode; title: string; detail: string; command: string; busy: string | null; onRun: (command: string) => void }) { const running = busy === `command-${command}`; return <motion.article whileHover={{ x: 4 }} className="data-scan bg-[#0c0c0c] p-5"><div className="flex items-start justify-between gap-4"><div className="grid size-9 place-items-center border border-[#222222] bg-[#161616] text-[#ccf200]">{icon}</div><button disabled={running} onClick={() => onRun(command)} className="button-secondary h-8 px-3 text-xs">{running ? "Running…" : "Execute"}</button></div><h3 className="mt-4 text-sm font-black uppercase">{title}</h3><p className="mt-1.5 text-xs leading-5 text-[#9a9a9a]">{detail}</p></motion.article>; }
function Policy({ label: title, value }: { label: string; value: string }) { return <div className="flex items-center justify-between gap-4 py-3.5"><dt className="text-xs text-[#9a9a9a]">{title}</dt><dd className="text-right font-mono text-xs font-semibold">{value}</dd></div>; }
