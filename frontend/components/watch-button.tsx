...
export function WatchButton({ githubId, fullName }: { githubId: number; fullName: string }) {
  ...
  return <div className="space-y-3">
    <button className="button-primary" onClick={() => setOpen(!open)}>{status === "Watching" ? "Edit watch reason" : "Watch this project"}</button>
    {open && <form className="panel space-y-3 p-4" onSubmit={e => {
      ...
      <p className="text-xs text-[#9a9a9a]">Saved in this browser. Your reason and blocker stay on this device.</p>
      <button className="button-primary" type="submit">Save watch reason</button>
    </form>}
