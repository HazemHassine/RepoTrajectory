...
export function EvidenceList({ items }: { items: Evidence[] }) {
  ...
  </article>) : <p className="text-sm text-[#9a9a9a]">No normalized source evidence collected yet.</p>}</div>;
}
...
export function ProjectBrief({ brief }: { brief: Brief }) {
  return <section className="space-y-6" aria-label="Developer project brief">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h2 className="text-2xl font-bold">Is this worth trying?</h2>
        <p className="mt-1 text-sm text-[#9a9a9a]">Source facts and open questions. Deterministic brief · no generated recommendation.</p></div>
      <WatchButton githubId={brief.github_id} fullName={brief.full_name} />
    </div>
    <div className="grid gap-4 lg:grid-cols-2">
      <article className="panel space-y-4 p-5"><h3 className="text-lg font-semibold">What it is</h3>
        <FactValue fact={brief.description} />
        {brief.readme_excerpt.value && <details><summary className="cursor-pointer text-sm text-[#ccf200]">Read the project's own introduction</summary>
          <div className="mt-3 whitespace-pre-wrap text-sm"><FactValue fact={brief.readme_excerpt} /></div></details>}
      </article>
      <article className="panel space-y-4 p-5"><h3 className="text-lg font-semibold">How to try it</h3>
        ...
        <p className="text-sm text-[#9a9a9a]">First experiment: use the official quickstart to test your smallest required workflow. Record any missing requirement as a watch blocker.</p>
        <Link className="button-secondary" href={`/compare?a=${encodeURIComponent(brief.full_name)}`}>Compare for my situation</Link>
      </article>
    </div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <article className="panel p-5"><h3 className="mb-4 text-lg font-semibold">Check before deciding</h3>
        <ul className="space-y-3 text-sm text-[#bbb]">{brief.missing.map((item, i) => <li key={i}>{item}</li>)}</ul></article>
    </div>
    <details className="panel p-5"><summary className="cursor-pointer text-lg font-semibold">Inspect releases, issues, security & discussion evidence ({brief.evidence.length})</summary>
      <div className="mt-5"><EvidenceList items={brief.evidence} /></div></details>
    <details className="panel p-5"><summary className="cursor-pointer text-sm">Source health & provenance</summary>
      ...
      </div>) : <p>External enrichment has not run. Only catalog metadata is available.</p>}</div>
    </details>
  </section>;
}
